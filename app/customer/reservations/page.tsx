    ...
    <main className="flex flex-1 flex-col bg-muted/20">
      <div className="mx-auto flex w-full max-w-7xl flex-1 flex-col gap-6 p-4 md:p-6 lg:p-8">
        ...
        <Card className="bg-background">
          {step === 0 && <MenuStep packages={packages} loading={catalogLoading} cart={cart} onAddToCart={addToCart} onToggleCart={toggleCart} onChange={setPackageChoice} onCustomize={(id, title, details, quantity) => { setPackageChoice(title); setNotes(details); addToCart(id); changeCartQuantity(id, quantity) }} />}
          ...
          {step === 3 && <ReviewStep packages={packages} cart={cart} quantities={cartQuantities} eventType={eventType} eventDate={eventDate} guests={guests} location={location} packageChoice={packageChoice} notes={notes} eventDetails={eventDetails} />}
...
function ReviewStep({ packages, cart, quantities, eventType, eventDate, guests, location, packageChoice, notes, eventDetails }: { packages: ReservationPackage[]; cart: string[]; quantities: Record<string, number>; eventType: string; eventDate: string; guests: string; location: string; packageChoice: string; notes: string; eventDetails: Record<string, string> }) {
