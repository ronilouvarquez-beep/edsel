...
const packages = [
  ...
  { title: "Anniversary cake", category: "Desserts", categories: ["Desserts"], description: "A layered celebration cake styled for anniversaries", price: "From ₱980", includes: ["Custom cake flavor", "Personalized message", "Dessert accents"], images: ["https://images.unsplash.com/photo-1588195538326-c5b1e2c8f2f1?auto=format&fit=crop&w=900&q=85"] },
  { title: "Corporate lunch buffet", category: "Catering Buffet", categories: ["Catering Buffet"], description: "A complete lunch spread ready for meetings and gatherings", price: "From ₱5,800", includes: ["Savory trays", "Rice and sides", "Drinks"], images: ["https://images.unsplash.com/photo-1546069901-ba9599a7e63c?auto=format&fit=crop&w=900&q=85"] },
  { title: "Pasta party tray", category: "Food", categories: ["Food"], description: "Creamy pasta prepared for sharing at the table", price: "From ₱1,250", includes: ["Creamy pasta", "Serving trays", "Portions by guest count"], images: ["https://images.unsplash.com/photo-1473093295043-cdd812d0e601?auto=format&fit=crop&w=900&q=85"] },
  { title: "Lechon kawali tray", category: "Food", categories: ["Food"], description: "Crispy pork belly with rice and atchara", price: "From ₱1,650", includes: ["Lechon kawali", "Steamed rice", "Atchara"], images: ["https://images.unsplash.com/photo-1432139555190-58524dae6a55?auto=format&fit=crop&w=900&q=85"] },
  { title: "Chocolate dessert bar", category: "Desserts", categories: ["Desserts"], description: "A dessert station with chocolate, fruit, and dippers", price: "From ₱2,450", includes: ["Chocolate fountain", "Fruit dippers", "Marshmallows"], images: ["https://images.unsplash.com/photo-1481391319762-47dff72954d9?auto=format&fit=crop&w=900&q=85"] },
]
...
function MenuStep({ cart, onAddToCart, onToggleCart, onChange, onCustomize }: { cart: string[]; onAddToCart: (title: string) => void; onToggleCart: (title: string) => void; onChange: (value: string) => void; onCustomize: (title: string, details: string) => void }) {
  ...
  return <><CardHeader className="relative flex flex-col gap-4"><div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between"><div><CardTitle>Select your menu</CardTitle><CardDescription>Select one or more menus, then customize any item before continuing.</CardDescription></div><Badge variant="secondary" className="w-fit shrink-0 self-end sm:absolute sm:top-6 sm:right-6"><ShoppingCartIcon /> {cart.length} selected</Badge></div><div className="flex flex-col gap-3 xl:flex-row xl:items-center"><div className="relative w-full max-w-xs xl:w-[220px] xl:flex-none"><SearchIcon className="absolute top-1/2 left-3 size-4 -translate-y-1/2 text-muted-foreground" /><Input value={search} onChange={(event) => { setSearch(event.target.value); setVisibleCount(PAGE_SIZE) }} placeholder="Search menus or categories..." aria-label="Search reservation menus" className="h-9 pl-9" /></div><div className="flex min-w-0 flex-1 justify-end overflow-x-auto pb-1"><div className="flex min-w-max items-center gap-2">{categories.map((category) => <button key={category} type="button" onClick={() => { setSelectedCategory(category); setVisibleCount(PAGE_SIZE) }} className={`shrink-0 rounded-full border px-3 py-1.5 text-xs font-medium leading-none transition-colors ${selectedCategory === category ? "border-primary bg-primary text-primary-foreground" : "border-border bg-background text-foreground hover:bg-muted"}`} aria-pressed={selectedCategory === category}>{category}</button>)}</div></div></div></CardHeader><CardContent><div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">{visiblePackages.map((item) => <MenuChoice key={item.title} item={item} selected={cart.includes(item.title)} inCart={cart.includes(item.title)} onSelect={(title) => { onChange(title); onToggleCart(title) }} onAddToCart={onAddToCart} onCustomize={onCustomize} />)}{filteredPackages.length === 0 && <div className="rounded-xl border border-dashed p-8 text-center sm:col-span-2 lg:col-span-3"><p className="font-medium">No menus found</p><p className="mt-1 text-sm text-muted-foreground">Try another menu or category.</p></div>}</div>{filteredPackages.length > PAGE_SIZE && <div className="mt-6 flex justify-center"><Button variant="outline" onPress={() => setVisibleCount((current) => hasMore ? current + PAGE_SIZE : PAGE_SIZE)}>{hasMore ? `See more (${filteredPackages.length - visibleCount} left)` : "Show less"}</Button></div>}</CardContent></>
}
